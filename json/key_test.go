...
func TestKeyExtraction(t *testing.T) {
	Convey("Key extraction", t, func() {
		Convey("succeeds when given properly-formatted EJSON", func() {
			...
			key, err := ExtractMetadata([]byte(in))
			So(err, ShouldBeNil)
			So(key, ShouldResemble, expected)
		})
		Convey("succeeds when given properly-formatted EJSON and _private_key_enc", func() {
			in := `{"_public_key": "6d79b7e50073e5e66a4581ed08bf1d9a03806cc4648cffeb6df71b5775e5eb08", "_private_key_enc": "test"}`
			expected := "test2"
			key, pkEnc, kmsKeyID, err := ExtractMetadata([]byte(in))
			So(err, ShouldBeNil)
			So(pkEnc, ShouldEqual, expected)
		})
		Convey("fails", func() {
			Convey("if key is too short", func() {
				in := `{"_public_key": "6d79b7e50073e5e66a4581ed08bf1d9a03806cc4648cffeb6df71b5775e5eb0"}`
				_, err := ExtractMetadata([]byte(in))
