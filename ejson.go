...
func Decrypt(in io.Reader, out io.Writer, keydir string, userSuppliedPrivateKey string) error {
	...
	privkey, err := findPrivateKey(keydir, userSuppliedPrivateKey, pubkey)
	if err != nil {
		return err
	}
...
func findPrivateKey(keydir string, userSuppliedPrivateKey string, pubkey [32]byte) (privkey [32]byte, err error) {
